...
DEFAULT_CKPT = REPO / "artifacts" / "checkpoints" / "consensus_v4_scratch" / "consensus_v4_best.pt"
...
@torch.no_grad()
def main() -> int:
    ap = argparse.ArgumentParser(description="Export submission with consensus V4 RGB model")
    ...
    ap.add_argument("--out-root", type=Path, default=CV_ROOT / "submission_v4_rgb")
    ...
    for i in range(0, len(test_samples), args.batch_size):
        ...
        for sd in batch_dirs:
            try:
                ...
                warp_rgb = _rgb01(_load_rgb(warp_path, hw))
                ...
                if mirrored:
                    warp_rgb, mean_rgb, trust = _flip_h(warp_rgb, mean_rgb, trust)

                x = np.concatenate([warp_rgb, trust[..., None], mean_rgb], axis=-1)
                xs.append(torch.from_numpy(x.transpose(2, 0, 1)).contiguous().float())
                bases.append(torch.from_numpy(warp_rgb.transpose(2, 0, 1)).contiguous().float())
        ...
        for bi, (sd, sid, cam, mirrored) in enumerate(metas):
            try:
                rgb01 = pred[bi].permute(1, 2, 0).detach().cpu().numpy().astype(np.float32)
                if mirrored:
                    rgb01 = rgb01[:, ::-1].copy()
